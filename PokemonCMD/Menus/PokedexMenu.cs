...
namespace PokemonCMD
{
    class PokedexMenu : BaseMenu
    {
        ...
        public override void ShowMenu()
        {
            ...
            TextPrinter.Print($"Navigate pages via arrow keys. and Acess specific pokemons via the the numbers");
            ...
            {
                MenuManager.CurrentMenu = new SpecificPokemonMenu(pokemonReference[userInput]);
            }
            else
            {
                switch (userInput)
        ...
        private void ShowPokemons(List<Pokemon> userPokemons)
        {
            ...
            for (int i = indexStart; i < userPokemons.Count; i++)
            {
                if (i < indexStart + 9)
                {
                    TextPrinter.Print($"[{i + 1 - indexStart}] {userPokemons[i].Name} {userPokemons[i].Level} ");
